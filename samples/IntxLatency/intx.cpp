...
#define LOG_TAG_INTX_SAMPLE "[Sample - IntxLatency]"
...
namespace awsiotsdk {
    namespace samples {
        bool IntxThing::ConnectivitySortFunction(ConnectivityInfo info1, ConnectivityInfo info2) {
        ...
        ResponseCode IntxThing::ActionResponseHandler(util::String thing_name, ShadowRequestType request_type,
        ...
        ResponseCode IntxThing::RunSample() {
            ...
            if (ResponseCode::SUCCESS != rc) {
                AWS_LOG_ERROR(LOG_TAG_INTX_SAMPLE,
            ...
            do {
                ...
                if (rc != ResponseCode::DISCOVER_ACTION_SUCCESS) {
                    ...
                    if (rc != ResponseCode::DISCOVER_ACTION_NO_INFORMATION_PRESENT) {
                        AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE,
                                     "Discover Request failed with response code: %d.  Trying again...",
                                     static_cast<int>(rc));
                        std::this_thread::sleep_for(std::chrono::seconds(5));
                    } else {
                        AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE, "No GGC connectivity information present for this Device: %d",
                                     static_cast<int>(rc));
                        return rc;
                    }
                }
            ...
            if (max_retries == DISCOVER_ACTION_RETRY_COUNT) {
                AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE, "Discover failed after max retries, exiting");
                return rc;
            }

            AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE, "GGC connectivity information found for this Device! %d\n",
                         static_cast<int>(rc));
            ...
            std::sort(parsed_response.begin(), parsed_response.end(), std::bind(IntxThing::ConnectivitySortFunction,
                                                                                std::placeholders::_1,
                                                                                std::placeholders::_2));
            ...
            for (auto connectivity_info_itr : parsed_response) {
                ...
                for (auto ca_list_itr: ca_map_itr->second) {
                    ...
                    AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE, "Using CA at : %s\n", core_ca_file_path.c_str());
                    ...
                    AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE, "Connect attempt failed with this CA!!");
                    suffix_itr++;
                }
                if (ResponseCode::MQTT_CONNACK_CONNECTION_ACCEPTED == rc) {
                    AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE, "Connected to GGC %s in Group %s!!",
                                 connectivity_info_itr.ggc_name_.c_str(),
                                 connectivity_info_itr.group_name_.c_str());
                    break;
                }
                AWS_LOG_INFO(LOG_TAG_INTX_SAMPLE, "Connect attempt failed for GGC %s in Group %s!!",
                             connectivity_info_itr.ggc_name_.c_str(),
                             connectivity_info_itr.group_name_.c_str());
            }
            ...
            std::unique_lock <std::mutex> block_handler_lock(sync_action_response_lock_);
            {
                ...
                Shadow::RequestHandlerPtr p_action_handler =
                    std::bind(&IntxThing::ActionResponseHandler, this, std::placeholders::_1,
                              std::placeholders::_2,
                ...
                if (ResponseCode::SUCCESS != rc) {
                    AWS_LOG_ERROR(LOG_TAG_INTX_SAMPLE, "Json Parse for template failed with return code : %d",
                                  static_cast<int>(rc));
                ...
                if (ResponseCode::SHADOW_REQUEST_REJECTED == rc) {
                    AWS_LOG_ERROR(LOG_TAG_INTX_SAMPLE, "Shadow update failed with return code : %d",
                                  static_cast<int>(rc));
                    return rc;
                }
		...
		// ceate pipe name INTX_PIPE = "intxpipe"
#define INTX_PIPE "/demo/intxpipe"
#if 0
		if (mkfifo(INTX_PIPE, 0777) < 0)
			printf("failed to create pipe\n");
		else
			printf("create pipe: %s .... done!\n", INTX_PIPE);
#endif                                

		std::cout << "------- Invoke interrupt latency programming --------" << std::endl << currentState
			  << std::endl;
		char cmd[1024];
		sprintf(cmd, "run_interrupt.sh &");
		system(cmd);
...
#if 0
				//open this pipe
				int fd = open(INTX_PIPE, O_WRONLY);
				if (fd == -1)
					printf("failed to open pipe: %s\n", INTX_PIPE);
				else
					printf("Open pipe: %s .... okay\n", INTX_PIPE);
				//unlinke(INTX_PIPE);
#endif
...
int main(int argc, char **argv) {
    ...
    std::unique_ptr <awsiotsdk::samples::IntxThing>
        intxlatency = std::unique_ptr<awsiotsdk::samples::IntxThing>(new awsiotsdk::samples::IntxThing());
    awsiotsdk::util::Logging::InitializeAWSLogging(p_log_system);

    awsiotsdk::ResponseCode rc = awsiotsdk::ConfigCommon::InitializeCommon("config/IntxConfig.json");
    if (awsiotsdk::ResponseCode::SUCCESS == rc) {
        rc = intxlatency->RunSample();
    }

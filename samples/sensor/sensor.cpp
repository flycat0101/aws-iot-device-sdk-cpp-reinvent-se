...
#define THING_NAME_TO_UPDATE "aws-actuator-thing"

#define LOG_TAG_SENSOR_SAMPLE "[Sample - sensor]"
...
namespace awsiotsdk {
    namespace samples {
        bool sensorThing::ConnectivitySortFunction(ConnectivityInfo info1, ConnectivityInfo info2) {
        ...
        ResponseCode sensorThing::RunSample() {
            ...
            if (ResponseCode::SUCCESS != rc) {
                AWS_LOG_ERROR(LOG_TAG_SENSOR_SAMPLE,
            ...
            do {
                ...
                if (rc != ResponseCode::DISCOVER_ACTION_SUCCESS) {
                    ...
                    if (ResponseCode::DISCOVER_ACTION_NO_INFORMATION_PRESENT != rc) {
                        AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE,
                                     "Discover Request failed with response code : %d.  Trying again...",
                                     static_cast<int>(rc));
                        std::this_thread::sleep_for(std::chrono::seconds(5));
                    } else {
                        AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE, "No GGC connectivity information present for this Device");
                        return rc;
                    }
                }
            ...
            if (max_retries == DISCOVER_ACTION_RETRY_COUNT) {
                AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE, "Discover failed after max retries, exiting");
                return rc;
            }

            AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE, "GGC connectivity information found for this Device!!\n");
            ...
            std::sort(parsed_response.begin(), parsed_response.end(), std::bind(sensorThing::ConnectivitySortFunction,
                                                                                std::placeholders::_1,
                                                                                std::placeholders::_2));
            ...
            for (auto connectivity_info_itr : parsed_response) {
                ...
                for (auto ca_list_itr: ca_map_itr->second) {
                    ...
                    AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE, "Using CA at : %s\n", core_ca_file_path.c_str());
                    ...
                    AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE, "Connect attempt failed with this CA!!");
                    suffix_itr++;
                }
                if (ResponseCode::MQTT_CONNACK_CONNECTION_ACCEPTED == rc) {
                    AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE, "Connected to GGC %s in Group %s!!",
                                 connectivity_info_itr.ggc_name_.c_str(),
                                 connectivity_info_itr.group_name_.c_str());
                    break;
                }
                AWS_LOG_INFO(LOG_TAG_SENSOR_SAMPLE, "Connect attempt failed for GGC %s in Group %s!!",
                             connectivity_info_itr.ggc_name_.c_str(),
                             connectivity_info_itr.group_name_.c_str());
            }
...
*/  
          // Rapidjson uses move semantics, doc needs to be initialized again
            ...
            if (ResponseCode::SUCCESS != rc) {
                AWS_LOG_ERROR(LOG_TAG_SENSOR_SAMPLE, "Json Parse for sample failed with return code : %d",
                              static_cast<int>(rc));
...
int main(int argc, char **argv) {
    ...
    std::unique_ptr <awsiotsdk::samples::sensorThing>
        sensor_thing = std::unique_ptr<awsiotsdk::samples::sensorThing>(new awsiotsdk::samples::sensorThing());
    awsiotsdk::util::Logging::InitializeAWSLogging(p_log_system);

    awsiotsdk::ResponseCode rc = awsiotsdk::ConfigCommon::InitializeCommon("config/sensorConfig.json");
    if (awsiotsdk::ResponseCode::SUCCESS == rc) {
        rc = sensor_thing->RunSample();
    }
